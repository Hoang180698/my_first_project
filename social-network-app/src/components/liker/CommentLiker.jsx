...
import { useLazyGetUserLikeCommentQuery} from "../../app/services/user.service";

var pageSize = 10
function CommentLiker({ onHide, show , commentId }) {
    const [getUsers] = useLazyGetUserLikeCommentQuery();
    ...
    useEffect(() => {
      if(currentPage > 0 && !isLast) {
        setLoading(true);
        getUsers({commentId: commentId, page: currentPage, pageSize: pageSize}).unwrap()
        .then((data) => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          let { data } = await getUsers({commentId: commentId, page:0, pageSize: pageSize});
